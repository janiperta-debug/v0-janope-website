...
export default async function RakennusPage({ params }: { params: Promise<{ slug: string; building: string }> }) {
  ...
  return (
    <Panel>
      ...
      <EmblemHeading icon={building.icon} accentVar={area.accentVar} title={building.name} tagline={building.tagline} />
      ...
      {building.link ? <PrimaryLink href={building.link} label={building.linkText} /> : <span className="map-kicker inline-flex w-fit items-center rounded-lg border border-dashed border-border px-6 py-4 text-xs text-muted-foreground">{building.linkText}</span>}
